    ...
    soundManager(new SoundManager()) // SoundManager 객체 초기화
{
    ...
    // 음악 재생
    soundManager->playMusic("Musics/Megalovania 8Bit Remix Audio.mp3.mp3", -1); // 음악 파일 경로와 반복 횟수 설정
}
...
void TetrisScene::render()
{
    ...
    if (heartVisible) {
        print->printPNG("heartNote.png", heartPosX, 370, 1); // 하트 노드 이미지 파일을 사용하세요
    }
